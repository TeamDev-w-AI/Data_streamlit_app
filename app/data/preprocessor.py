...
class TimeSeriesPreprocessor:
    ...
    def create_features(self, df, target_col, lags=None, add_date_features=True):
        ...
        Args:
            ...
            add_date_features (bool): Whether to add date-based features
    ...
    def create_sequences(self, scaled_data, sequence_length, batch_size, val_split=0.2, test_split=0.2):
        """
        Create TensorFlow datasets with proper sizing based on sequences.
        
        Args:
            scaled_data (np.array): Scaled input data
            sequence_length (int): Length of input sequences
            batch_size (int): Batch size for training
            val_split (float): Validation split proportion
            test_split (float): Test split proportion
